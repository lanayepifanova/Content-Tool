...
## Bucket B — Tutorials (AI workflows & tools)

Looking for: something Lana could actually build and film, where the payoff is
visible on screen.

**Named interests:** useful open-source GitHub repos · automating video editing
with Claude + Premiere Pro · an AI agent that scrolls X · Mobbin MCP · Figma
Weave · Firecrawl · MCP servers generally · agent orchestration.

**Strong signals:** the result is visually obvious in a screen recording ·
buildable in under an hour · solves a chore the audience recognizes · the tool
is new enough that nobody's covered it well · there's a real "before/after."

**Weak signals:** requires an API key wall or paid tier to even demo · the payoff
...
**Strong signals:** there's a crisp physical or everyday analogy that doesn't
lie · the common intuition is actually wrong and correcting it is satisfying ·
